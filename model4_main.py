...
train_file = [filepath + "/imgdata/train/oridata/csvfile/train_" + str(i) + ".csv"  for i in range(1, 29)]
valid_file = [filepath + "/imgdata/valid/oridata/valid_" + str(i) + ".csv" for i in range(1, 4)]
...
train_img, train_label = concat_data(train_file)
valid_img, valid_label = concat_data(valid_file)
...
train_dataset = tf.data.Dataset.from_tensor_slices((trainimage, trainlabel)).shuffle(buffer_size = 50000).repeat(20).batch(batch_size)
...
with tf.name_scope("Optimizer"):
    ...
    momentum = tf.train.exponential_decay(init_momentum, global_step, 2000, 0.9, staircase = True)
...
#        pass
    ...
    for itr in range(num_iterations):
        ...
        if itr % saver_step == 0:
            saver.save(sess, checkpoint_path + "/model3_r2_Iter_" + str(itr+1)+ ".ckpt",
                    global_step = global_step)

        if itr % csv_steps == 0:
            data_df = pd.DataFrame(data_dict)
            data_df.to_csv(datafile_path + "/model3_r2_" + str(itr) + ".csv")
            pre_df = pd.DataFrame(pre_dict)
            pre_df.to_csv(datafile_path + "/model3_r2_pre_" + str(itr) + ".csv")
