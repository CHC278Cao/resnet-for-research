...
class Resnet(object):
    def __init__(self, inputs, num_classes, imgsize, is_training):
        ...
        self._buildModel(self.is_training)
    ...
    def _buildModel(self, train_mode):
        ...
        with tf.variable_scope("conv4"):
            conv4 = resdual_v1(conv3, 32, 32, 32, train_mode)
            activation_summary(conv4)
            
        # feature map is 32x32x64
        with tf.variable_scope("conv5"):
            conv5 = resdual_v2(conv4, 32, 32, 64, train_mode)
            activation_summary(conv5)
        
        # feature map is 16x16x64
        with tf.variable_scope("conv6"):
            conv6 = resdual_v1(conv5, 16, 16, 32, train_mode)
